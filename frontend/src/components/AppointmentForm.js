...
const AppointmentForm = () => {
    ...
    const concerns = [{title: 'Skin and hair'}, {title: 'Mom and child health'}, {title: 'General wellness'}, {title: 'Nutrition'}, 
    {title: 'Medical reports opinion'}, {title: 'Healthy living'}, {title: 'Mother and Child Counselling - 4 Sessions', amount: 1299},
    {title: 'Mental Health Care - 5 Sessions', amount: 5099}, {title: 'Physical Health - 4 Sessions', amount: 1299}, 
    {title: 'Nutrition Care - 4 Follow Ups', amount: 1299}]

    const getAmount = () => {
      let temp;
      if (['Mother and Child Counselling - 4 Sessions','Mental Health Care - 5 Sessions', 'Physical Health - 4 Sessions', 'Nutrition Care - 4 Follow Ups'].includes(concern)){
        temp = concerns.find(x => x.title === concern)?.amount
      }else temp = newUser && video ? 399 : 199
      return temp
    }
    ...
    const handleSubmit = async (e) => {
        ...
        if(!name || !dob || !gender || !phone || !concern || !dateTime || (age < 18 && !guardian) ){
      ...
      switch(step) {
          ...
          <div className="w-full md:w-1/2">
            <div className="space-y-2 mb-2">
              ...
              <select name="concern" id="concern" autoFocus={true}
                ...
                {
                  concerns.map((x, i) => (
                    <option key={'concern' + i} className="px-4 py-2 border border-gray-300 text-gray-600 transition duration-300 focus:ring-2 focus:ring-sky-300 focus:outline-none" value={x.title}>{x.title}</option>
                  ))
                }</select>
            </div>
        ...
        case 4: return <div className='w-full flex flex-col items-center justify-center'>
          ...
          <div id="coupon-sec" style={{display:"flex",flexDirection:"column",alignItems:"center",marginTop:"40px",gap:"20px"}}>
            ...
            <h3 className="text-lg lg:-mt-2"> Amount: <span className="font-bold">{getAmount()}</span> | {coupondata && <>Discount: <span className="font-bold"> {coupondata.value || 0}{coupondata?.type === 'percentage' ? '%' : '' } </span></>} | Total: <span className="font-bold text-[#E94C60]">{Math.round(amount)}</span></h3>
          </div>
